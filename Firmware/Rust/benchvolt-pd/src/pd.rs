...
const CANONICAL_NVM_SECTOR3: [u8; 8] = [0x00, 0x19, 0x55, 0xaa, 0x57, 0x35, 0x5f, 0x00];
...
fn read_nvm_sector(bus: &mut impl PdBus, sector: u8) -> Result<[u8; 8], PdError> {
    ...
    bus.write(FTP_CTRL1, &[0]).map_err(|_| PdError::Bus)?;
    bus.write(FTP_CTRL0, &[0xd0 | sector])
        .map_err(|_| PdError::Bus)?;
    ftp_wait(bus)?;
    let mut data = [0; 8];
    bus.read(FTP_BUFFER, &mut data).map_err(|_| PdError::Bus)?;
    bus.write(FTP_CTRL0, &[0]).map_err(|_| PdError::Bus)?;
    ftp_exit(bus)?;
    Ok(data)
}

fn write_nvm_sector(bus: &mut impl PdBus, sector: u8, data: &[u8; 8]) -> Result<(), PdError> {
    bus.write(FTP_PASSWORD, &[0x47]).map_err(|_| PdError::Bus)?;
    bus.write(FTP_BUFFER, &[0]).map_err(|_| PdError::Bus)?;
    bus.write(FTP_CTRL0, &[0]).map_err(|_| PdError::Bus)?;
    bus.write(FTP_CTRL0, &[0xc0]).map_err(|_| PdError::Bus)?;
    for opcode in [((1 << sector) << 3) | 0x02, 0x07, 0x05] {
        bus.write(FTP_CTRL1, &[opcode]).map_err(|_| PdError::Bus)?;
        bus.write(FTP_CTRL0, &[0xd0]).map_err(|_| PdError::Bus)?;
        ftp_wait(bus)?;
    }
    bus.write(FTP_BUFFER, data).map_err(|_| PdError::Bus)?;
    for (opcode, control) in [(0x01, 0xd0), (0x06, 0xd0 | sector)] {
        bus.write(FTP_CTRL0, &[0xc0]).map_err(|_| PdError::Bus)?;
        bus.write(FTP_CTRL1, &[opcode]).map_err(|_| PdError::Bus)?;
...
/// Apply one modification to an NVM sector. Only the targeted sector is
/// erased, unrelated bytes are preserved, and the programmed sector is read
/// back before success is returned. An erased sector is first replaced with
/// the canonical image. `apply` mutates the sector in place and returns true
/// when the sector already held the requested configuration.
fn update_nvm_sector(
    bus: &mut impl PdBus,
    sector: u8,
    canonical: [u8; 8],
    apply: &mut dyn FnMut(&mut [u8; 8]) -> bool,
) -> Result<NvmUpdate, PdError> {
    let mut data = read_nvm_sector(bus, sector)?;
    let erased = data == [0xff; 8];
    if erased {
        data = canonical;
    }
    if apply(&mut data) && !erased {
        return Ok(NvmUpdate::AlreadyConfigured);
    }
    write_nvm_sector(bus, sector, &data)?;
    if read_nvm_sector(bus, sector)? != data {
        return Err(PdError::ContractMismatch);
    }
    Ok(NvmUpdate::Updated)
}

fn configure_nvm_bit(
    bus: &mut impl PdBus,
    sector: u8,
    byte_index: usize,
    bit_mask: u8,
    ...
    update_nvm_sector(bus, sector, canonical, &mut |data| {
        let already = data[byte_index] & bit_mask != 0;
...
/// effect at the STUSB4500's next cold attach.
pub fn configure_usb_comm_capable(bus: &mut impl PdBus) -> Result<NvmUpdate, PdError> {
    configure_nvm_bit(bus, 3, 2, 0x01, CANONICAL_NVM_SECTOR3)
}
...
/// Synchronously read the source's advertised PDOs for the desktop GUI's
/// `SOUR:PD:LIST?` query: require attach and sink-ready, transmit
/// Get_Source_Cap, and poll for the capabilities message. Bounded polling;
/// non-capability messages are skipped.
pub fn read_source_capabilities(bus: &mut impl PdBus) -> Result<([u32; 7], usize), PdError> {
    if read_register(bus, PORT_STATUS)? & 1 == 0 || read_register(bus, PE_FSM)? != PE_SINK_READY {
    ...
    bus.write(COMMAND_CTRL, &[SEND_COMMAND])
        .map_err(|_| PdError::Bus)?;
    for _ in 0..500 {
        if read_register(bus, PRT_STATUS)? & PD_MESSAGE_RECEIVED == 0 {
            continue;
        }
        ...
        let byte_count = read_register(bus, RX_BYTE_COUNT)?;
        let count = usize::from((u16::from_le_bytes(header) >> 12) & 0x07);
        let mut bytes = [0u8; 28];
        let length = bytes.len().min(count * 4);
        bus.read(RX_DATA, &mut bytes[..length])
            .map_err(|_| PdError::Bus)?;
        // decode_source_capabilities re-validates the header, object count,
        // and byte count before accepting the frame.
        return decode_source_capabilities(header, byte_count, &bytes[..length]);
    }
    Err(PdError::Timeout)
}

/// Persist the preferred sink voltage as NVM PDO2 so the STUSB4500's
/// autonomous negotiation requests it at every cold attach. A live downward
/// contract transition reboots this VBUS-powered board, erasing any volatile
/// RAM PDO — the NVM is the only state the re-attach negotiation reads, so it
/// must be written before the renegotiation is triggered. Current is not
/// stored; `REQ_SRC_CURRENT` mode requests the full advertised current.
pub fn configure_nvm_sink_voltage(
    bus: &mut impl PdBus,
    millivolts: u16,
) -> Result<NvmUpdate, PdError> {
    if !(5_000..=20_000).contains(&millivolts) || millivolts / 50 * 50 != millivolts {
        return Err(PdError::NoSuitablePdo);
    }
    let units = millivolts / 50;
    update_nvm_sector(bus, 4, CANONICAL_NVM_SECTOR4, &mut |sector| {
        let already = (u16::from(sector[1]) << 2) | u16::from(sector[0] >> 6) == units;
        sector[0] = (sector[0] & 0x3f) | (((units & 0x03) as u8) << 6);
        sector[1] = (units >> 2) as u8;
        already
    })
}

/// Apply the GUI's `SOUR:PDO:SET`: for the slot-3 override, first persist the
/// voltage to NVM PDO2 (see [`configure_nvm_sink_voltage`]), then write the
/// volatile RAM sink PDO and re-advertise source capabilities so the STUSB4500
/// re-runs its autonomous match. The archived C firmware sent a PD Soft Reset
/// instead, which drops VBUS on some sources before the volatile PDO is used.
///
/// `DPM_PDO_NUMB` must cover the written slot or the chip never evaluates it —
/// this board's NVM loads a count of 2, which silently disabled the C
/// firmware's slot-3 override. The highest matching slot wins the match, so an
/// enabled slot 3 acts as the override.
pub fn set_sink_pdo(
    bus: &mut impl PdBus,
    slot: u8,
    millivolts: u16,
    ...
    let pdo = encode_sink_fixed_pdo(millivolts, milliamps).ok_or(PdError::NoSuitablePdo)?;
    if slot == 3 {
        configure_nvm_sink_voltage(bus, millivolts)?;
        bus.write(SINK_PDO_COUNT, &[3]).map_err(|_| PdError::Bus)?;
    }
    bus.write(SINK_PDO1 + 4 * (slot - 1), &pdo.to_le_bytes())
        .map_err(|_| PdError::Bus)?;
    bus.write(TX_HEADER, &GET_SOURCE_CAPABILITIES)
        .map_err(|_| PdError::Bus)?;
    bus.write(COMMAND_CTRL, &[SEND_COMMAND])
