        ...
        I: IntoIterator<Item = Pixel<Self::Color>>,
    {
        ...
        for Pixel(point, color) in pixels {
            if (0..WIDTH as i32).contains(&point.x) && (0..HEIGHT as i32).contains(&point.y) {
                buf[point.y as usize * WIDTH + point.x as usize] = RawU16::from(color).into_inner();
            }
        }
        Ok(())
    }
...
fn base_state() -> AppState {
    ...
    // On hardware the monitoring loop samples every channel every 20 ms, so an
    // idle channel reads a valid 0.00 V / 0.00 A. The "--.--" placeholder only
    // appears when an ADC conversion actually fails.
    for channel in &mut state.channels {
        channel.measurement = measurement(0, 0);
    }
    state
}
...
impl Recorder {
    ...
    fn detent(&mut self, direction: i8) {
        ...
        let (raw, accelerated) = clamp_adjustment(raw, accelerated);
...
fn menu_flow(out_dir: &Path) {
    ...
    // Feed telemetry the way the firmware monitoring loop would: it runs from
    // boot regardless of screen, so every channel already has a valid reading
    // before the user ever leaves the main menu.
    rec.dispatch(Action::Measurements([measurement(0, 0); 5]));
    ...
    let mut samples = [measurement(0, 0); 5];
    ...
    for rgb in &rec.frames {
        let data = rgb.clone();
        let mut frame =
            gif::Frame::from_rgb_speed((WIDTH * SCALE) as u16, (HEIGHT * SCALE) as u16, &data, 10);
...
fn main() {
    // tools/render/src -> tools -> benchvolt-pd -> Rust -> Firmware -> repo root.
