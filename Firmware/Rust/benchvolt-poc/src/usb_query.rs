...
pub fn dispatch_command(
    ...
    match command {
        b"*IDN?" => reply(out, "BenchVolt-PD,RUST-POC,S/N:2026-01\r\n"),
        b"SYST:BUILD?" => reply(out, "Rust POC 0.1.0\r\n"),
        b"SYST:HWERR?" => {
            let mut response = Response::new_empty();
            write!(
                &mut response,
...
mod tests {
    ...
    fn identification_build_and_unknown_commands_reply_directly() {
        ...
        assert_eq!(reply, "BenchVolt-PD,RUST-POC,S/N:2026-01\r\n");
        ...
        assert_eq!(reply, "Rust POC 0.1.0\r\n");
    }
