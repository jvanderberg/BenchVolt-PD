...
// The bootloader reads only the seal words at offsets 0..8 of the metadata
// page and erases the whole page before writing a new seal, so the page tail
// is free for append-only markers that survive VBUS power cycles.
const PD_MARKER_BASE: usize = BOOT_METADATA_ADDR + 8;
const PD_MARKER_SLOTS: usize = (FLASH_PAGE_SIZE - 8) / 2;
const PD_MARKER: u16 = 0xa55a;

fn pd_marker_count() -> usize {
    (0..PD_MARKER_SLOTS)
        .take_while(|slot| {
            let value: u16 =
                unsafe { core::ptr::read_volatile((PD_MARKER_BASE + slot * 2) as *const u16) };
            value != u16::MAX
        })
        .count()
}

/// True when the boot-time PD renegotiation may transmit. An odd marker count
/// means the previous renegotiation attempt ended in a reset before it could
/// settle, so this boot must not retrigger it; a full marker region fails
/// closed the same way until a firmware upload erases the page.
pub(crate) fn pd_renegotiation_allowed() -> bool {
    let count = pd_marker_count();
    count.is_multiple_of(2) && count < PD_MARKER_SLOTS
}

/// True when an attempt marker has no matching settle marker: either this
/// boot transmitted a request, or a previous attempt ended in a reset.
pub(crate) fn pd_attempt_pending() -> bool {
    !pd_marker_count().is_multiple_of(2)
}

/// Append one marker: before transmitting a renegotiation request, and again
/// once the boot contract settles, so a completed cycle leaves an even count.
pub(crate) fn record_pd_marker() -> bool {
    ...
    let slot = pd_marker_count();
    if slot >= PD_MARKER_SLOTS {
        return false;
    }
    let address = PD_MARKER_BASE + slot * 2;
    unsafe {
        ...
        core::ptr::write_volatile(address as *mut u16, PD_MARKER);
        let ready = benchvolt_wait_for_flash_ready(SR);
        let ok = ready && core::ptr::read_volatile(SR) & SR_ERRORS == 0;
        core::ptr::write_volatile(CR, (core::ptr::read_volatile(CR) & !CR_PG) | CR_LOCK);
        ok && core::ptr::read_volatile(address as *const u16) == PD_MARKER
    }
}
