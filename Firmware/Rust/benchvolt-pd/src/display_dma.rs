...
use benchvolt_pd::paint_queue::{dma_deadline_reached, PaintCommand, PaintQueue, PaintTransfer};
...
impl QueuedDisplay {
    ...
    fn enqueue(command: PaintCommand) {
        match LIFECYCLE.load(Ordering::Acquire) {
            LIFECYCLE_AWAIT_CONTINUE => return,
...
pub fn install<T>(resources: T) {
    ...
    LIFECYCLE.store(LIFECYCLE_AWAIT_CONTINUE, Ordering::Release);
...
pub fn lifecycle_label() -> &'static str {
    match LIFECYCLE.load(Ordering::Acquire) {
        ...
        LIFECYCLE_AWAIT_CONTINUE => "READY",
