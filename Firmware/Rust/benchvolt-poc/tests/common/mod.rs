...
use benchvolt_poc::monitoring::ProtectionService;
...
pub fn eligible_contract() -> benchvolt_poc::pd::Contract {
    benchvolt_poc::pd::Contract {
        source_position: 1,
        millivolts: 5_000,
...
pub fn assert_invariants(harness: &Harness) {
    ...
    for (index, output) in state.channels.iter().enumerate() {
        if let benchvolt_poc::app::OutputTransition::Enabling(_) = output.transition {
            assert!(
