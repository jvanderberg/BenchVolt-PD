...
fn parse_pdo_set(args: &[u8]) -> Result<Option<UsbIntent>, CommandError> {
    let mut fields = args
        .split(|byte| *byte == b' ')
        .filter(|field| !field.is_empty());
    let mut next_value = |limit: u16| -> Result<u16, CommandError> {
        let field = fields.next().ok_or(CommandError::Syntax)?;
    ...
    let slot = next_value(3)?;
    if slot == 0 {
        return Err(CommandError::Range);
    }
    let millivolts = next_value(20_000)?;
    let milliamps = next_value(5_000)?;
    Ok(Some(UsbIntent::PdoSet {
        slot: slot as u8,
        millivolts,
        milliamps,
    }))
...
pub fn parse_compat_mutation(command: &[u8]) -> Result<Option<UsbIntent>, CommandError> {
    ...
    if command == b"SOUR:PD:LIST?" {
        return Ok(Some(UsbIntent::PdList));
    }
    if let Some(args) = command.strip_prefix(b"SOUR:PDO:SET ") {
        return parse_pdo_set(args);
    }
...
mod tests {
    ...
    #[test]
    fn gui_pdo_list_and_set_commands_parse_like_the_original_firmware() {
        assert_eq!(
            parse_compat_mutation(b"SOUR:PD:LIST?"),
            Ok(Some(UsbIntent::PdList))
        );
        assert_eq!(
            parse_compat_mutation(b"SOUR:PDO:SET 3 15000 3000"),
            Ok(Some(UsbIntent::PdoSet {
                slot: 3,
                millivolts: 15_000,
                milliamps: 3_000,
        ...
        assert_eq!(
            parse_compat_mutation(b"SOUR:PDO:SET 0 15000 3000"),
            Err(CommandError::Range)
        );
        assert_eq!(
            parse_compat_mutation(b"SOUR:PDO:SET 3 21000 3000"),
            Err(CommandError::Range)
        );
        assert_eq!(
            parse_compat_mutation(b"SOUR:PDO:SET 3 15000"),
            Err(CommandError::Syntax)
        );
    }
