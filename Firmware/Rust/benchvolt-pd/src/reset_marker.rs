...
use benchvolt_pd::reset_cause::{ResetMarker, ResetReason, MARKER_TAG};
...
pub(crate) unsafe fn take(reset_causes: u8, ram_parity_disabled: bool) -> Option<ResetReason> {
    if !benchvolt_pd::reset_cause::retained_marker_read_allowed(reset_causes, ram_parity_disabled) {
